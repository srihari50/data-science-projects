...
if uploaded_file is not None:     
    ...
    g = nx.from_pandas_edgelist(data, source = "SOURCE", target = "DESTINATION", edge_attr=['DISTANCE_KM','SPEED','DURATION_HRS']) #Use the Graph API to create an empty network graph object
    ...
    shortest_route = nx.shortest_path(g, source = SOURCE, target = DESTINATION)
    
    if st.button("Click here for shortest_route"):
